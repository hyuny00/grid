class TreeGridManager {
    ...
    bindEvents() {
        ...
        $(`#${this.gridId}-container`).on('change', '.row-check', function(e) {
            ...
            // 체크박스 상태 변경을 데이터에 반영
            const nodeId = String($checkbox.closest('tr').data('id'));
 ...
 // ★  전체 유효성 검사 오류 목록 업데이트
    ...
    renderTemplate(templateId, data, mapData, selectOption, reverseIndex) {
        ...
        // 2단계: 복합 조건문 파서 (개수 제한 없음)
        // 복합 조건문 파싱 함수
        function parseComplexCondition(conditionStr) {
            // 'and'로 조건들을 분리
            const conditions = conditionStr.split(/\s+and\s+/);
        ...
        // 복합 조건문만 먼저 처리 - else 있는 버전 (and가 포함된 경우만)
        result = result.replace(
            /\{\{#if\s+([^}]+?)\s+and\s+([^}]+?)\}\}([\s\S]*?)\{\{else\}\}([\s\S]*?)\{\{\/if\}\}/g,
            function(match, firstPart, restPart, trueContent, falseContent) {
                const conditionStr = firstPart + ' and ' + restPart;
        ...
        // 복합 조건문만 먼저 처리 - else 없는 버전 (and가 포함된 경우만)
        result = result.replace(
            /\{\{#if\s+([^}]+?)\s+and\s+([^}]+?)\}\}([\s\S]*?)\{\{\/if\}\}/g,
            function(match, firstPart, restPart, content) {
                const conditionStr = firstPart + ' and ' + restPart;
                const conditions = parseComplexCondition(conditionStr);
                if (conditions.length > 0) {
                    const result = evaluateConditions(conditions);
