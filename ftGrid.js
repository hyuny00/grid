class TreeGridManager {
    constructor(config) {
       ...
       // this.isSaveEnabled = !!(this.urls.saveExlUrl || this.urls.saveUrl);
        ...
        // 그리드별 독립적인 데이터
        // initialPage 설정: config에서 전달받거나, 폼의 currentPage 값을 읽거나, 기본값 1
        const formCurrentPage = config.searchFormId ?
            parseInt($(`#${config.searchFormId} input[name="currentPage"]`).val()) : null;
        this.currentPage = config.initialPage || (formCurrentPage && formCurrentPage > 0 ? formCurrentPage : 1);
        this.data = [];
     ...
     // 행 더블클릭 콜백 함수
        ...
        this.effectCheckBox = config.effectCheckBox || 'Y';
		// boolean 그리드 정렬시 재조회 하지 않고 현재 데이터 정렬 여부. 기본값은 false (재조회)
		this.sortNoSearch = config.sortNoSearch || false;
		// 그리드 리사이저 적용 default : Y
		this.resizableYn = config.resizableYn || 'Y';
		// select2 적용 여부 (기본값: false)
		this.enableSelect2 = config.enableSelect2 || false;
    ...
    selectRow(rowId, isMultiple = false) {
        if (!isMultiple) {
           ...
           // $(`#${this.gridId}-body tr`).removeClass('selected');
            $(`#${this.gridId}-body tr`).removeClass('on');
        }
       ...
       // $(`#${this.gridId}-body tr[data-id="${rowId}"]`).addClass('selected');
        $(`#${this.gridId}-body tr[data-id="${rowId}"]`).addClass('on');
    ...
    setEffectCheckBox(effect) {
    	this.effectCheckBox=effect;
    }
    ...
    init() {
    	...
    	if( this.isExcelMode){
    		 ...
    		 this.bindEvents();
    	 	 ...
    	 	 if( this.isLoading=='Y'){
    	 	 	// constructor에서 설정된 currentPage로 데이터 조회
    	 	 	// 초기 로딩 플래그 설정 (searchData가 페이지를 리셋하지 않도록)
    ...
    bindEvents() {
        ...
        $(`#${this.gridId}-container .btn-move-up`).on('click', function() {
            self.moveRowUp();
        });

       $(`#${this.gridId}-container .btn-move-down`).on('click', function() {
            self.moveRowDown();
        });
    ...
    bindCheckBoxEvents() {
        const self = this;
        const $checkAllCheckbox = $(`#${this.gridId}-container .check-all`);
        if ($checkAllCheckbox.length > 0) {
            $checkAllCheckbox.off('change').on('change', function() {
                console.log('CHECK-ALL 이벤트 바인딩됨 - checked:', this.checked);
                self.toggleAll(this.checked);
            });
        }
    }
    ...
    getSearchParams() {
        ...
        Object.keys(this.gridData).forEach(key => {
        	...
        	/*
        	if(typeof value != 'undefined'  && value !== null && value !=='undefined' && value.toString().trim()!==''){
    ...
    searchData(page) {
        ...
        console.log('[searchData] 호출됨, 현재 페이지:', this.currentPage, '요청 페이지:', page);
        // page 파라미터가 없으면 1로 리셋 (검색 버튼 클릭 시)
        // page 파라미터가 있으면 해당 페이지 유지 (목록 복귀 시)
    ...
    resetSearchData() {
        ...
        if( this.isExcelMode){
        	...
        	if(this.isLoading=='Y'){
    	        ...
    	        tbody.empty();
    	        const template = document.getElementById(this.templateId).innerHTML;

    	        const tdCount = (template.match(/<td/g) || []).length;
    	        console.log("tdCount",tdCount);

    	        tbody.append(`<tr><td class="empty" colspan=${tdCount}><p>데이터가 없습니다.</p></td></tr>`);
    ...
    fetchData(page = 1) {
        ...
        console.log('[fetchData] 호출됨, 요청 페이지:', page);
        this.currentPage = page;

        // 페이징이 활성화된 경우, 폼의 currentPage 값을 업데이트
        if (this.isPagingEnabled) {
            $(`#${this.searchFormId} input[name="currentPage"]`).val(this.currentPage);
        }
 ...
 // 2. renderTable 메서드 수정 - 병합 로직 추가
    renderTable() {
        ...
        $(`#${this.gridId}-container [data-value]`).each(function() {
            ...
            if (value !== undefined && value !== null) {
                ...
                } else if ($element.is('[contenteditable]')) {  //editId=20250904  contenteditable 처리위해추가
                    // contenteditable 요소 처리
                    const stringValue = String(value);
                    // HTML 태그가 포함된 경우와 일반 텍스트 구분
                    if (stringValue.includes('<') && stringValue.includes('>')) {
                        $element.html(stringValue);
                    } else {
    ...
    bindEditEvents() {
        ...
        $(`#${this.gridId}-body`)
            ...
            .on('click.treeToggle', '.btn-tblnode', function(e) {
                e.preventDefault();
                const nodeId = String($(this).closest('tr').data('id'));
                if (nodeId) {
        ...
        // 5. contenteditable의 Enter 키 처리 (선택사항)
        $(`#${this.gridId}-body`)
            .off('keydown.contenteditable')
            .on('keydown.contenteditable', '[contenteditable][data-field]', function(e) {
                const $this = $(this);

                // Enter 키 처리 (필요에 따라 수정)
                if (e.keyCode === 13) {
                    // 기본적으로는 줄바꿈 허용, 필요시 preventDefault() 호출
                     e.preventDefault();
                    // 편집 추적
                    const nodeId = String($this.closest('tr').data('id'));
                    const field = $this.data('field');
        ...
        // 6. contenteditable 포커스 아웃 시 추가 처리 (선택사항)
        $(`#${this.gridId}-body`)
            .off('focusout.contenteditable')
            .on('focusout.contenteditable', '[contenteditable][data-field]', function(e) {
                const $this = $(this);
                const nodeId = String($this.closest('tr').data('id'));
                const field = $this.data('field');
        ...
        // 7. Select2 초기화 (enableSelect2 옵션이 true인 경우만)
        if (this.enableSelect2 && typeof $.fn.select2 === 'function') {
            const gridId = this.gridId;
            $(`#${this.gridId}-body select[data-field]`).each(function() {
                const $select = $(this);

                // 옵션이 있는지 확인
                if ($select.find('option').length === 0) {
                    console.warn('Select2: 옵션이 없습니다', $select);
                    return;
                }

                // 이미 select2가 적용되어 있으면 제거 후 재적용
                ...
                $select.select2({
                    ...
                    dropdownParent: $(`#${gridId}-container`)
                });
            });
        }
    }
 ...
 // 5. createNodeRow 메서드 수정 - 병합 정보 포함
    createNodeRow(node, isVisible, index, reverseIndex = null, mergeInfo = {}) {
        ...
        if (this.isTreeMode) {
            ...
            toggleSymbol = hasChildren ? (node.treeExpanded ? '<button type="button" class="btn-tblnode opened" title="닫기"></button>' : '<button type="button" class="btn-tblnode" title="열기"></button>') : '<span class="space"></span>';
            if (node.level > 0 && hasChildren === false) toggleSymbol = '<span class="line"></span>';
        ...
        if (node.level) {
        	for(var i=0;i < node.level; i++) {
        		toggleSymbol = '<span class="space"></span>' + toggleSymbol;
        	}
        }
 ...
 findLastChildIndex(parentIndex) {
    ...
    addChildToSelected() {
      ...
      //  this.addChildRow(parentId);
        ...
        return this.addSelectedChildRow(parentId);
    }

    addSelectedChildRow(parentId) {
    	 // 트리 모드가 아니면 실행하지 않음
        if (!this.isTreeMode) return;
    ...
    // 체크박스 상태 적용 (렌더링 후 호출용)
    applyCheckboxState() {
        if (this.checkboxEnabled !== undefined) {
            const $container = $(`#${this.gridId}-container`);
            const $allCheckboxes = $container.find('.row-check');
            const $headerCheckbox = $container.find('.check-all');

            $allCheckboxes.prop('disabled', !this.checkboxEnabled);
            $headerCheckbox.prop('disabled', !this.checkboxEnabled);
        }
    }
    ...
    trackEdit(nodeId, el, field) {
        ...
        //editId=20250904  contenteditable 처리위해추가
        const $el = $(el);
        ...
        if (el.type === 'checkbox') {
            ...
            value = el.value;
        	  ...
        	  //editId=20250904  contenteditable 처리위해추가
            // contenteditable 요소 처리
            // HTML 형태로 저장할지, 텍스트만 저장할지는 설정에 따라 결정
            // 기본적으로는 HTML 형태로 저장하되, 순수 텍스트가 필요한 경우 .text() 사용
            value = $el.html().trim();

            // 만약 순수 텍스트만 필요한 경우 아래 주석을 해제하고 위 라인을 주석 처리
            // value = $el.text().trim();

            // 빈 HTML 태그만 남은 경우 처리 (예: <br>, <div><br></div> 등)
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = value;
            const textContent = (tempDiv.textContent || tempDiv.innerText || '').trim();
            if (!textContent && value.match(/^(<br\s*\/?>|<div><br\s*\/?>(<\/div>)?|\s*)$/i)) {
                value = '';
            }
        ...
        if (result && result.node) {
            ...
            if (this.isExcelMode && result.node.isExcel) {
                ...
                if (nodeIndex >= 0 && nodeIndex < this.excelData.length) {
                    //this.excelData[nodeIndex][field] = value;
                	 ...
                	 if ($el.is('[contenteditable]')) {
                         // HTML에서 순수 텍스트 추출하여 엑셀 데이터에 저장
                         const tempDiv = document.createElement('div');
                         tempDiv.innerHTML = value;
                         this.excelData[nodeIndex][field] = (tempDiv.textContent || tempDiv.innerText || '').trim();
                     } else {
                         this.excelData[nodeIndex][field] = value;
                     }
                ...
                // 실시간 유효성 검사 - contenteditable의 경우 텍스트 값으로 검증
                //editId=20250904  contenteditable 처리위해추가
                let validationValue = value;
                if ($el.is('[contenteditable]')) {
                    const tempDiv = document.createElement('div');
                    tempDiv.innerHTML = value;
                    validationValue = (tempDiv.textContent || tempDiv.innerText || '').trim();
                }


                // 실시간 유효성 검사
                //editId=20250904  contenteditable 처리위해 수정, 입력파라메터 value ->validationValue 로 수정
                const validationError = this.validateSingleCell(nodeId, field, validationValue);
 ...
 // ★  전체 유효성 검사 오류 목록 업데이트
            ...
            phone: function(value) {
                ...
                if (onlyNumber.startsWith('02')) {
                    if (onlyNumber.length === 9) {
                        return onlyNumber.replace(/(\d{2})(\d{3})(\d{4})/, '$1-$2-$3');
            ...
            function(match, variable, formatType, formatOptions) {
                ...
                if (formatHelpers[formatType]) {
                    ...
                    if (formatType === 'date') {
                        ...
                        result = formatHelpers[formatType](value);
                    }
                    return result;
                }
    ...
    bindRowClickEvents() {
        ...
        $(`#${this.gridId}-body`).off('click.rowClick').on('click.rowClick', 'tr', function(e) {
            // 체크박스, 버튼, input 등은 제외
            if ($(e.target).is('input, button, select, .btn-tblnode')) {
                return;
            }
...
//특정 행의 데이터를 업데이트하는 메서드
 ...
 bindSingleRowEvents($row) {
	    ...
	    // 트리 토글 이벤트
	    $row.on('click', '.btn-tblnode', function(e) {
	        e.preventDefault();
	        const nodeId = String($(this).closest('tr').data('id'));
	        if (nodeId) {
	            self.toggleTree(nodeId, e);
...
//행 데이터를 클립보드에 복사하는 메서드
  ...
  fallbackCopyToClipboard(text) {
	...
	moveRowUp() {
	    ...
	    const selectedRow = checkedRows.closest("tr");
	    const nodeId = String(selectedRow.data("id"));
	...
	moveNode(nodeId, direction) {
	    ...
	    const node = result.node;
	    let targetArray = this.data;

	    // 트리 모드인 경우 부모의 children 배열을 사용
	    if (this.isTreeMode && result.parent) {
	        targetArray = result.parent.children;
	    }

	    const currentIndex = targetArray.indexOf(node);
	    if (currentIndex === -1) return;

	    let newIndex;
	    if (direction === 'up') {
	        if (currentIndex === 0) {
	            alert("첫 번째 항목은 위로 이동할 수 없습니다.");
	            return;
	        }
	        newIndex = currentIndex - 1;
	    } else {
	        if (currentIndex === targetArray.length - 1) {
	            alert("마지막 항목은 아래로 이동할 수 없습니다.");
	            return;
	        }
	        newIndex = currentIndex + 1;
	    }
	    ...
	    targetArray.splice(newIndex, 0, node);

	    // 저장 기능이 활성화된 경우 수정된 행으로 추적
	    /*
	    if (this.isSaveEnabled) {
	        const nodeKey = this.getNodeKey(node);
	        this.modifiedRows.add(nodeKey);

	        // 이동의 영향을 받는 다른 노드도 수정된 것으로 표시
	        const affectedNode = targetArray[direction === 'up' ? newIndex + 1 : newIndex - 1];
	        if (affectedNode) {
	            const affectedNodeKey = this.getNodeKey(affectedNode);
	            this.modifiedRows.add(affectedNodeKey);
	        }
	    }
	    */
	...
	addSortIcons() {
		$(`#${this.gridId}-container table th.sortable`).each(function() {
			...
			if (nHasChk === 0) {
				...
				if (nHasSortIcon === 0) {
					const sHtml = ($comp.html() || '') + '<span class="sort-icon"></span>';
					$comp.html(sHtml);
				}
			}
		});
	 ...
	 */
	setTableId() {
		const $table = $(`#${this.gridId}-container table`);
		if ($table && $table.length && !$table.attr('id')) {
			$table.attr('id', this.gridId);
		}
	}

	/**
	 * 그리드 리사이저 적용.
	 * this.resizableYn : default 'Y'
	 * gridResizer.js
	 */
	addResizer() {
		const $table = $(`#${this.gridId}-container table`);

		if (!$table.length || $table.hasClass('resizable-table') || 'N' === this.resizableYn) {
			return;
		}
		...
		$(`#${this.gridId}-container table th`).each(function() {
			const $comp = $(this);
//			const nHasChk = $comp.find('input[type=checkbox]').length;
//			const booRowNo = 'NO' === $comp.text();
			const nHasResizer = $comp.find('.resizer').length;
			if (nHasResizer === 0) {
				const sHtml = ($comp.html() || '') + '<div class="resizer"></div>';
				$comp.html(sHtml);
			}
		});

		// gridResizer.js
		const domTable = document.getElementById(tbid);
		globalResizer.initializeTable(domTable);
	}
